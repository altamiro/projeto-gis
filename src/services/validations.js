...
export default {
   ...
   */
  async validatePropertyIntersectsMunicipality(geometry, municipalityName) {
    try {
      // Em uma aplicação real, isso buscaria a geometria do município em um serviço
      const municipalityGeometry = await this.getMunicipalityGeometry(municipalityName);
      
      // Verificar interseção
      const intersects = arcgisService.intersects(geometry, municipalityGeometry);
      
      return {
        success: intersects,
        message: intersects 
          ? null 
          : `A área do imóvel deve intersectar o município ${municipalityName}.`
      };
    } catch (error) {
  ...
  async validatePropertyInSaoPaulo(geometry) {
    try {
      ...
      if (!intersection) {
        return {
          success: false,
          message: 'A área do imóvel deve estar, ao menos parcialmente, no estado de São Paulo.'
        };
      }
  ...
  validateGeometryInsideProperty(geometry, propertyGeometry) {
    ...
    const isInside = arcgisService.geometryEngine.within(geometry, propertyGeometry);
    
    return {
      success: isInside,
      message: isInside ? null : 'A geometria deve estar completamente dentro da área do imóvel.'
    };
  },
