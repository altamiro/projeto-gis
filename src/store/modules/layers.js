export default {
  ...
  state: {
    ...
    drawingMode: false,
    layerTypes: [
      { id: 'propertyArea', name: 'Área do imóvel', order: 1, editable: true, required: true },
      { id: 'headquarters', name: 'Sede do imóvel', order: 2, editable: true, required: false },
      { id: 'consolidatedArea', name: 'Área consolidada', order: 3, editable: true, required: false },
      { id: 'nativeVegetation', name: 'Remanescente de vegetação nativa', order: 4, editable: true, required: false },
      { id: 'fallow', name: 'Área de pousio', order: 5, editable: true, required: false },
      { id: 'administrativeServitude', name: 'Servidão administrativa', order: 6, editable: true, required: false },
      { id: 'hydrography', name: 'Hidrografia', order: 7, editable: false, required: false },
      { id: 'anthropizedAfter2008', name: 'Área Antropizada após 2008', order: 8, editable: false, required: false }
    ]
  },
  mutations: {
  ...
  actions: {
    selectLayer({ commit, state, rootState }, layerId) {
      ...
      const propertyAreaDefined = state.layers.some(l => l.id === 'propertyArea');
      ...
      if (layerId !== 'propertyArea' && !propertyAreaDefined) {
        rootState.validation.alerts.push({
          type: 'error',
          message: 'É necessário definir a Área do imóvel antes de outras camadas.'
        });
        return false;
      }
    ...
    async addLayer({ commit, dispatch, state, rootState }, { layerId, geometry }) {
      ...
      if (['propertyArea', 'nativeVegetation', 'consolidatedArea', 'administrativeServitude', 'hydrography'].includes(layerId)) {
      ...
      switch (layerId) {
        case 'propertyArea': {
          ...
          if (!isInSaoPaulo) {
            return { 
              success: false, 
              message: 'A maior parte da área do imóvel deve estar dentro do estado de São Paulo.' 
            };
          }
        ...
        case 'headquarters': {
          ...
          // Simulação - verificar se está dentro
          const isInsideProperty = true;
          ...
          const hydrographyLayers = state.layers.filter(l => l.type === 'hydrography');
          const overlapsHydrography = false; // Simulação
          
          if (overlapsHydrography) {
            return { 
              success: false, 
              message: 'A sede do imóvel não pode sobrepor áreas de hidrografia.' 
            };
          }
        ...
        case 'consolidatedArea': {
          ...
          const overlapsNativeVegetation = false; // Simulação
          const overlapsServitude = false; // Simulação
          const overlapsHydrography = false; // Simulação
          
          if (overlapsNativeVegetation) {
            return { 
              success: false, 
          ...
          if (overlapsServitude) {
            return { 
              success: false, 
              message: 'A área consolidada não pode sobrepor servidões administrativas.' 
            };
          }
          ...
          if (overlapsHydrography) {
            return { 
              success: false, 
              message: 'A área consolidada não pode sobrepor áreas de hidrografia.' 
            };
          }
  ...
  getters: {
    ...
    isPropertyFullyCovered(state) {
      ...
      const coveredArea = layers
        .filter(l => ['consolidatedArea', 'nativeVegetation', 'fallow', 'anthropizedAfter2008'].includes(l.id))
